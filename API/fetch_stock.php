...
// Function to process stock request
function processStockRequest($request) {
    ...
    if (!$stockData || empty($stockData['body']) || !isset($stockData['body'][0])) {
        return ["status" => "error", "message" => "Ticker not found in the API response"];
    }

    $foundStock = $stockData['body'][0];
    ...
    // Send the stock data to the database consumer for storage
    $dbClient = new rabbitMQClient("/home/dmz/IT490-Project/testRabbitMQ_response.ini", "responseServer");
    $dbResponse = $dbClient->send_request($dataToStore);

    if ($dbResponse['status'] === 'success') {
        return ["status" => "success", "data" => $dataToStore['data']];
    } else {
        return ["status" => "error", "message" => "Failed to store stock data in database."];
    }
}
